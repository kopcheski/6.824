...
func (rf *Raft) ticker() {
	var tickerTimeout = time.Duration(10) * time.Second
	for !rf.killed() {
		if time.Since(rf.lastPing) > tickerTimeout {
			rf.fireElection()
		}

		time.Sleep(tickerTimeout) // FIXME it should be random
	}
}
